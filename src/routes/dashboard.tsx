...
import { ClipboardList, Pencil, Plus, Target, Trophy, Users } from 'lucide-react'
...
function DashboardPage() {
  ...
  const allResponses = stripSystemFieldsAll<SurveyResponse>(
    useQuery(api.responses.listAll, ready ? {} : 'skip') as never,
  )
  ...
  const teams = useMemo<TeamRow[]>(() => {
    if (!surveys) return []
    const byTeam = new Map<string, SurveyResponse[]>()
    for (const response of allResponses ?? []) {
      const list = byTeam.get(response.questionnaireId) ?? []
      list.push(response)
      byTeam.set(response.questionnaireId, list)
    }
    return surveys
      .map((questionnaire) => ({
        questionnaire,
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="max-w-xl text-muted-foreground">
            One team per survey. See who has collected how much, keep the leaderboard moving, and
            talk to the team.
          </p>
        </div>
            ...
            <section className="space-y-5">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 space-y-1">
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        ...
                        {activeEnumerators(selected.questionnaire).length === 0 && (
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        ...
                        nativeButton={false}
                        render={
                          <Link to="/surveys/$surveyId/fill" params={{ surveyId: selected.questionnaire.id }} />
                        }
                      >
                        Open survey
                      </Button>
              ...
              <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem] xl:items-start">
                <div className="space-y-3">
                  ...
                  <Leaderboard members={selected.stats.members} now={now} />
                </div>
                <TeamChat questionnaire={selected.questionnaire} now={now} />
              </div>
            </section>
          )}
        </div>
